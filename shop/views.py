    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            try:
                ...
                context['in_cart'] = Cart.objects.filter(customer=customer, car=self.object).exists()
...
class CustomerUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        if self.kwargs.get('pk'):  # Если передан pk в URL
            return get_object_or_404(Customer, pk=self.kwargs['pk'])
        return self.request.user.customer  # Иначе возвращаем профиль текущего пользователя

    def get_success_url(self):
        if self.kwargs.get('pk'):  # Если редактируем другого пользователя
            return reverse_lazy('customer_list')
        return reverse_lazy('account')  # Если редактируем свой профиль
...
class EmployeeUpdateView(AdminRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        user = form.save(commit=False)
        user.is_staff = True  # Убеждаемся, что пользователь остается сотрудником
        user.save()
        return super().form_valid(form)
...
# AJAX представление для загрузки моделей
...
def checkout(request):
    try:
        ...
        if request.method == 'POST':
            # Создаем продажу для каждого автомобиля в корзине
